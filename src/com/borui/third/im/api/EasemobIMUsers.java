...
import com.borui.third.im.vo.ClientSecretCredentail;
...
import com.borui.utils.MD5Util;
...
public class EasemobIMUsers {
	...
	//private static final String APPKEY ="pusun#pupumoney";
	...
	/*public static void main(String[] args) {
		...
		userNode.put("username", "test");
		userNode.put("password", "123456");
		
		createNewIMUserSingle(userNode);
	}*/
	/**
	 * 注册IM用户[单个]
	 * 
...
//			Credentail credentail = new UsernamePasswordCredentail("pupu",
//					"pupumoney", "appAdmin");
			
			Credentail credentail = new ClientSecretCredentail(Constants.APP_CLIENT_ID,
...
//					.resolveTemplate("app_name", APPKEY.split("#")[1]).path(userPrimaryKey);
			//URL queryUserPrimaryUrl = HTTPClientUtils.getURL(Constants.APPKEY.replace("#", "/") + "/users/"+ username);
			webTarget = EndPoints.USERS_TARGET.resolveTemplate("org_name", APPKEY.split("#")[0])
					.resolveTemplate("app_name", APPKEY.split("#")[1]).queryParam("/" + userPrimaryKey);
	...
	public static void main(String[] args) {
		...
		ObjectMapper mapper = new ObjectMapper();
		ObjectNode userNode = mapper.createObjectNode();
		userNode.put("username", "13222222222");
		userNode.put("password",MD5Util.MD5Encrypt("123456"));
		//objectNode = em.createNewIMUserSingle(userNode);
		
	//	getIMUsersByPrimaryKey("13545256767");
		
		objectNode =  imUserLogin("1990",MD5Util.MD5Encrypt("13579"));
		System.out.println(objectNode);
	}
